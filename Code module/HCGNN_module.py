...
class HCGNN(torch.nn.Module):
    def __init__(self, input_channels, hidden_channels, output_channels, dropout=0.5):
        super(HCGNN, self).__init__()
        self.conv1 = HeteroConv({
            ('asset', 'correlates_with', 'asset'): GCNConv(input_channels, hidden_channels)
        }, aggr='sum')
        self.conv2 = HeteroConv({
            ('asset', 'correlates_with', 'asset'): GCNConv(hidden_channels, hidden_channels)
        }, aggr='sum')
...
class ChangePointDetector:
    def __init__(self, file_path, graph_path, graph_prefix, graph_type='correlation'):
        ...
        if self.train_hetero_graphs:
            ...
            output_channels = 1
            self.model = HCGNN(input_channels, self.hidden_channels, output_channels)
